...
def CktModSetup(DSSfile,sectional_swt,tie_swt,generators): # give tie switches and sectionalizing switches as input
    ...
    for tline in tie_swt: # First create new lines corresponding to tie lines
        if tline['name']=='Sw7' or tline['name']=='Sw8':  
            DSSCktobj.dss.Text.Command(f"New Line.{tline['name']} Bus1={tline['from node']}{tline['from conn']} Bus2={tline['to node']}{tline['to conn']} r1=1e-3 r0=1e-3  x1=0.000 x0=0.000 c1=0.000 c0=0.000 Length=0.001")
        else:
            DSSCktobj.dss.Text.Command(f"New Line.{tline['name']} Bus1={tline['from node']}{tline['from conn']} Bus2={tline['to node']}{tline['to conn']} LineCode={tline['code']} Length={str(tline['length'])}")

        DSSCktobj.dss.Text.Command(f"New swtcontrol.swTie{str(tline['no'])} SwitchedObj=Line.{tline['name']} Normal=o SwitchedTerm=1 Action=o") #normally open  
