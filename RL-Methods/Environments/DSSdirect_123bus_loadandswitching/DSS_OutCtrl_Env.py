...
class DSS_OutCtrl_Env(gym.Env):
    ...
    def reset(self):
        ...
        max_percfail = 0.8  # Percentage of edges failed within subgraph -variable
        nd = random.choice(list(G_init.nodes()))  # select initial node around which subgraph is formed
        rad = ceil(uniform(0, max_rad / 3))  # select radius of subgraph from a uniform distribution
