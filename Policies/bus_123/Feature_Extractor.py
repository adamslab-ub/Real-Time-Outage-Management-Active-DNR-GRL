...
class CustomGNN(BaseFeaturesExtractor):
    ...
    def __init__(self,
        ...
        self.full_context_nn = torch.nn.Sequential(*[torch.nn.Linear(140, 2*features_dim), torch.nn.Linear(2*features_dim, features_dim)])
        self.switch_encoder = torch.nn.Sequential(*[torch.nn.Linear(130, 2*features_dim), torch.nn.Linear(2*features_dim, features_dim)])

        self.final_encoder = torch.nn.Sequential(*[torch.nn.Linear(2*features_dim, 4*features_dim), torch.nn.LeakyReLU(), torch.nn.Linear(4*features_dim, features_dim)])
    ...
    def forward(self, data):
        ...
        final = self.final_encoder(torch.cat((switch_embeddings.mean(dim=1),context), dim=-1))
